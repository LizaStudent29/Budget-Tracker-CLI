import {
  ...
  function createAccount(id: number, name: string): IAccount {
  ...
  // Создаём счёт
  const mainAccount: IAccount = createAccount(1, "Основной счёт");
  
  // Добавляем несколько транзакций
  mainAccount.addTransaction(
    createTransaction(1, 100000, "income", "Зарплата")
  );
  mainAccount.addTransaction(
    createTransaction(2, 15000, "expense", "Аренда квартиры")
  );
  mainAccount.addTransaction(
    createTransaction(3, 5000, "expense", "Еда")
  );
  mainAccount.addTransaction(
    createTransaction(4, 2000, "expense", "Подписки")
  );
  ...
  const summary: ISummary = accountManager.getSummary(1);
  console.log(summary);
